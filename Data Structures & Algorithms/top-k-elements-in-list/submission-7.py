class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # this is a normal hashmap that has the number as the key and the value as the frequency
        temp = {}
        for i in nums:
            temp[i] = 1 + temp.get(i,0)

        #this uses temp to revese so the key is now the frequency somthing occurs and the value is the numbers
        ...
        print(key)
        for i in range(len(key) -1 ,-1, -1): #start from biggest key (most ocurances)
            
            #need to go to -1 and not 0 since we stop at 1 if we put 0
            print(key[i])
